'''
This scripts converts all the .nd2 files in a directory into h5
files readable by Targettrack's GUI. Read nd22h5.py description.
run the script from terminal by:
python3 nd22h5List.py Allnd2s/directory/
where Allnd2s/directory/ is the direcory of the folder eincluding
all nd2 files. The nd2 files in the directory should have a name
in the format of YYYYMMDD_HHMMSS_etc.nd2 or at list one '_' character
in its name. The output files will be saved in the same directory with the name
HHMMSS.h5
'''
import sys
import os
