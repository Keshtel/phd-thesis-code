...
def plot_2mixedGaussian(data,b=200,label=True):
...
def plot_4mixedGaussian(data,b=200,label=True):
...
def plot_5mixedGaussian(data,b=200):
...
def compute_row_correlation(matrix, row_x, rows_y):
    row1 = matrix[:,row_x]
    ...
    for n in rows_y:
        ...
        row2 = compute_vector_z_score(matrix[:,n])#matrix[:,n]#
        row2[np.isnan(row2)] = 0
        # Compute correlation coefficient
        correlation[count] = np.corrcoef(row1[time_intersection], smooth_vec(compute_vector_z_score(row2[time_intersection]),2))[0, 1]
        correlation2[count] = scipy.stats.pearsonr(row1[time_intersection], compute_vector_z_score(row2[time_intersection]))[0]
